          ...
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl space-y-4">
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input value={icon} onChange={e => setIcon(e.target.value)} placeholder="e.g. 💳 or chart-bar"
                  className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:border-blue-400 focus:outline-none" />
              </div>
              {modal.mode === "edit" && (
                <div>
              ...
              <tbody className="divide-y divide-slate-50">
                {categories.map(cat => (
                  <tr key={cat.id} className="hover:bg-slate-50">
                    <td className="px-4 py-3 text-lg">{cat.icon || <span className="text-slate-300 text-xs">—</span>}</td>
                    <td className="px-4 py-3 font-medium text-slate-800">{cat.name}</td>
                    <td className="px-4 py-3 text-xs text-slate-500 max-w-[220px] truncate">{cat.description || <span className="text-slate-300">—</span>}</td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-1.5">
                        ...
                        <button onClick={() => setConfirm(cat)} disabled={(cat.startup_count ?? 0) > 0}
                          title={(cat.startup_count ?? 0) > 0 ? "Cannot delete — has startups" : "Delete category"}
