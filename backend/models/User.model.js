...
async function findById(id) {
  const [rows] = await db.execute(
    `SELECT ${PUBLIC_FIELDS} FROM users u LEFT JOIN profiles p ON p.user_id = u.id
...
async function findByEmailWithPassword(email) {
  const [rows] = await db.execute(
    `SELECT u.*, p.first_name, p.last_name, p.profile_photo, p.headline, p.verification_level
     FROM users u LEFT JOIN profiles p ON p.user_id = u.id
     WHERE LOWER(u.email) = ? LIMIT 1`, [normalizeEmail(email)]
  );
  return rows[0] || null;
}
...
async function countByRole() {
  const [rows] = await db.execute("SELECT role, COUNT(*) AS total FROM users GROUP BY role");
  return rows;
}

async function platformStats() {
  const [[startups]]   = await db.execute("SELECT COUNT(*) AS total FROM startups WHERE status != 'draft'");
  const [[investors]]  = await db.execute("SELECT COUNT(*) AS total FROM users WHERE role = 'investor' AND status = 'active'");
  const [[orgs]]       = await db.execute("SELECT COUNT(*) AS total FROM users WHERE role = 'organization' AND status = 'active'");
  const [[industries]] = await db.execute("SELECT COUNT(DISTINCT industry) AS total FROM startups WHERE industry IS NOT NULL AND status != 'draft'");
  const [[users]]      = await db.execute("SELECT COUNT(*) AS total FROM users WHERE status = 'active'");
  const [[investments]]= await db.execute("SELECT COUNT(*) AS total FROM investments WHERE status IN ('accepted','completed')");
  return {
    startups: startups.total, investors: investors.total, organizations: orgs.total,
    industries: industries.total, users: users.total, investments: investments.total,
  };
}

async function findPublicDirectory({ role, limit = 12, offset = 0 } = {}) {
  if (!role) throw new Error("role is required.");
  const safeLimit  = Math.max(1, Math.min(parseInt(limit,  10) || 12, 100));
  const safeOffset = Math.max(0, parseInt(offset, 10) || 0);
  const investorExtra = role === "investor"
    ? `, (SELECT COUNT(*) FROM investments i WHERE i.investor_id = u.id AND i.status IN ('accepted','completed','pending','negotiating')) AS investment_count`
    : "";
  const [rows] = await db.execute(
    `SELECT u.id, u.role, u.created_at,
            p.first_name, p.last_name, p.headline, p.bio,
            p.verification_level, p.country, p.profile_photo${investorExtra}
     FROM users u LEFT JOIN profiles p ON p.user_id = u.id
     WHERE u.role = ? AND u.status = 'active'
     ORDER BY CASE p.verification_level WHEN 'verified' THEN 0 WHEN 'premium' THEN 1 ELSE 2 END, u.created_at DESC
     LIMIT ${safeLimit} OFFSET ${safeOffset}`, [role]
  );
  return rows;
}

async function countActiveByRole(role) {
  const [[{ total }]] = await db.execute("SELECT COUNT(*) AS total FROM users WHERE role = ? AND status = 'active'", [role]);
  return total;
}
...
async function discover({ q, role, country, province, district, limit = 12, offset = 0 } = {}) {
  ...
  if (q && q.length <= 200) {
    ...
    conditions.push("(p.first_name LIKE ? OR p.last_name LIKE ? OR p.headline LIKE ? OR p.bio LIKE ?)");
    const pct = `%${safe}%`;
...
module.exports = {
  create, findById, findByUuid, findByEmailWithPassword, findAll,
  updateRole, updateStatus, updatePassword, setPasswordHash,
  markEmailVerified, markPhoneVerified, touchLastLogin, remove,
  countByRole, platformStats, findPublicDirectory,
  countActiveByRole, totalCount, discover,
};
